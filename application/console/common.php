 ...
 * @return string
 */
function manager_log($manager_id, $param = '',$operate='')
{
 ...
 * @return string
 */
function show_title($id, $name,$field='title')
{
    $title = db($name)->where('id',$id)->value($field);
    return $title;
}
/**
 * 用于操作按钮
...
function get_status($status, $imageShow = true)
{
    switch ($status) {
        ...
        case 1 :
            $showText = '正常';
            $showImg = '<i class = "glyphicon glyphicon-ok-sign text-success"></i>';
            break;
